...
function NewKeySecret({ secret, onDone }: { secret: string; onDone: () => void }) {
  ...
  return (
    <div className="rounded-lg border border-green-200 bg-green-50 p-4 mb-6">
      <p className="text-sm font-medium text-green-800 mb-2">Key 已创建，请立即复制保存，关闭后不再显示</p>
      <div className="flex items-center gap-2">
        <code className="flex-1 text-xs bg-white border border-green-200 rounded px-3 py-2 font-mono break-all select-all">
          {secret}
        </code>
        <button
          onClick={copy}
          className="shrink-0 text-xs bg-green-600 text-white px-3 py-2 rounded hover:bg-green-700"
        >
          {copied ? '已复制' : '复制'}
        </button>
      </div>
      <button onClick={onDone} className="mt-3 text-xs text-green-700 hover:underline">
        我已保存，关闭
      </button>
    </div>
  );
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-semibold text-gray-900">Publish Keys</h1>
          <p className="text-sm text-gray-500 mt-1">用于 CI/CD 和自动化工具发布 Skill，无需账号密码</p>
        </div>
        {!showForm && (
          <button
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="rounded-lg border border-gray-200 p-4 mb-6 bg-gray-50">
          <h2 className="text-sm font-medium text-gray-700 mb-3">新建 Publish Key</h2>
          <div className="space-y-3">
            <div>
              <label className="block text-xs text-gray-500 mb-1">Key 名称</label>
              <input
                type="text"
                value={formName}
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="text-sm text-gray-600 px-4 py-2 rounded hover:bg-gray-200"
              >
                取消
              </button>
            </div>
        ...
        <div className="border border-gray-200 rounded-lg overflow-hidden">
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                <th className="text-left text-xs font-medium text-gray-500 uppercase tracking-wider py-2 px-4">名称</th>
                <th className="text-left text-xs font-medium text-gray-500 uppercase tracking-wider py-2 px-4">状态</th>
                <th />
              </tr>
            </thead>
                ...
                <tr key={k.id} className={k.revokedAt ? 'opacity-50' : ''}>
                  ...
                  <td className="py-3 px-4 text-sm">
                    {k.revokedAt ? (
                      ...
                      <span className="text-yellow-600">已过期</span>
                    ) : (
      ...
      <div className="mt-8 rounded-lg border border-gray-200 bg-gray-50 p-4">
        ...
        <p className="text-xs text-gray-500 mb-2">将 Key 作为 Bearer Token 用于发布请求：</p>
        ...
        <p className="text-xs text-gray-400 mt-2">
          详见 <a href="/market" className="text-blue-500 hover:underline">/market</a> 页面的完整发布协议说明。
        </p>
      </div>
    </div>
  );
